...
**Facts verified:** 4 August 2026 — har claim ka source aur check date §15 ki table mein
...
5. [The Honest Finding — Roman Urdu ka Masla](#5-the-honest-finding--roman-urdu-ka-masla)
...
- [ ] Run a similarity comparison yourself with a 40-line script
...
### "Dimension" ka matlab — aur woh number kaun tay karta hai
...
### Ek vector kis cheez ka banta hai — document ka ya chunk ka?
...
> **Hamari book mein:** har lesson apni headings ke hisaab se chunks mein tootta hai — Lesson 02 ke ~15 sections taqreeban ~15 vectors bante hain.
...
Yeh lesson ka asal hissa hai — dawa nahi, **chalti hui cheez.** Humne ek 40-line script banayi jo 5 phrases embed kar ke har pair ka cosine score print karti hai. Model wohi jo hamara live tutor use karta hai.

🧑 **Step 1:** Book repo clone karen aur backend folder mein jayen (setup Lesson 03 mein detail se aayega — aaj sirf dekhna kaafi hai).

📋 **Step 2:** Script chalayen:

```bash
.venv/bin/python scripts/similarity_demo.py
```

**Asal output (4 August 2026 ko chalaya gaya — screenshots recording day per):**
...
`[screenshot: similarity-demo-output.png — capture on recording day]`

### Scores kya bata rahe hain
...
## 5. The Honest Finding — Roman Urdu ka Masla
...
> 🗣️ Roman Urdu: 3-small ke liye zubaan ka milna, matlab ke milne se bara signal ban gaya — yeh chhote model ki asal hadd hai.

Yeh koi bug nahi — yeh **chhote embedding model ki documented kamzori** hai. MIRACL (multilingual retrieval benchmark) per 3-small ka score **44.0%** hai, 3-large ka **54.9%**. *(Official — source §15)* Hamara demo usi farq ko apne data per pakar raha hai.
...
- Agar users **Roman Urdu / mixed language** mein poochenge aur content bhi mixed hai → yeh weakness seedha aapki retrieval quality ka masla ban jayegi — model choice ab business decision hai, agla section
...
*(Verified 4 August 2026 — pricing/models badalte rehte hain, recording se pehle dobara check karo)*
...
> **MTEB aur MIRACL kya hain?** Yeh embedding models ke **standardized imtihan** hain — jaise students ka board exam, waise models ka muqabla inhi se hota hai. **MTEB** zyada tar English tasks per; **MIRACL** 18 zubanon mein retrieval per: sawal ek zuban mein do, dekho model sahi document dhoond paata hai ya nahi. Yaad rakhne ki cheez sirf itni: **MIRACL = multilingual ka imtihan.**

### Faisla kaise karna hai
...
2. **Multilingual content ya users** → **3-large seriously consider karo.** MIRACL ka farq (44.0 → 54.9) woh jagah hai jahan large apne paise wasool karta hai — aur §5 ka demo dikhata hai ke yeh farq sirf benchmark ka nahi, aapke asal queries ka hai.
3. **Ek teesra option — lekin sirf bare paimane per.** Dono models ko `dimensions` parameter se **chhota kiya ja sakta hai.** Tafseel neeche.
...
### Vector chhota karna — Matryoshka (aur aapko iski zaroorat kab paregi)

**Matryoshka** woh Russian gurya hai jiske andar chhoti gurya, uske andar aur chhoti. OpenAI ne 3-small aur 3-large ko is tarah train kiya ke vector ke **shuru ke numbers sab se zyada matlab rakhte hain** — jaise bari gurya ke andar chhoti gurya bhi apne aap mein mukammal gurya hoti hai.

**Faida:** 3,072 numbers ke vector ko aap **256 per kaat** den, to bhi kaam chalta rehta hai.

⚠️ **"Kaatna" ka matlab divide karna ya compress karna nahi hai** — bilkul seedha matlab hai: shuru ke 256 numbers rakh lo, baaqi utha kar phenk do.

```
Poora vector (3,072 numbers):
[0.021, -0.045, 0.310, ... , 0.002, -0.019]
 └───── pehle 256 ─────┘   └── yeh 2,816 hata diye ──┘
```

Na koi average, na koi formula — **sirf list chhoti kar di.** Aam model mein aisa karna tabahi hota (matlab poore vector mein phaila hota hai), lekin Matryoshka training ne matlab ko shuru ke numbers mein **ghana** kar diya hai.

Address ki tarah samjhein: *"Pakistan, Karachi, Clifton, Block 5, House 12"* — aakhri hissa kaat den, phir bhi kaafi pata chal jata hai, kyuke sab se aham lafz **pehle** hain.

**OpenAI ka claim:** 3-large ko 256 dims tak kaat kar bhi woh purane `text-embedding-ada-002` (1,536 dims) se behtar rehta hai. *(Official — source §15)*

#### Lekin aapko iski zaroorat kab paregi?

Seedha jawab: **shayad kabhi nahi.** Yeh **scale ka tool** hai. Masla storage ka hai — har number ~4 bytes leta hai:

| Kitne chunks | Storage (1,536 dims) | Faisla |
|---|---|---|
| **~500** (hamari poori book) | ~3 MB | Sochna bhi nahi |
| **10,000** (bara client project) | ~60 MB | Sochna bhi nahi |
| **100,000** | ~600 MB | Ab dekhna shuru karo |
| **10 lakh+** | 6 GB+ | **Ab yeh parameter kaam ka hai** |

*(Yeh mera hisaab hai — koi official threshold nahi. 1 vector × 1,536 numbers × 4 bytes ≈ 6 KB.)*

Bare paimane per vectors **RAM mein** rehte hain taake search tez rahe — wahan 6 GB vs 1 GB har mahine ka bill ban jata hai. Aisi teams ke paas pehle do hi rastay the: sasta chahiye to chhota model (quality giri), quality chahiye to bara model (6× bill). Matryoshka ne teesra raasta diya — **bare model ki samajh, chhote vector ke kharche per.**

**Amal mein kaise:** embed karte waqt API call mein, baad mein nahi —

```python
client.embeddings.create(model="text-embedding-3-large",
                         input=text,
                         dimensions=256)   # 3,072 ki jagah 256
```

…aur Qdrant collection bhi 256 size ka banayen (Lesson 03).

> **Aapke liye ek line:** jab tak chunks lakh se neeche hain, `dimensions` ko haath mat lagayen — default hi sahi hai. Bas itna jaanna kaafi hai ke yeh cheez exist karti hai aur kis liye hai.
...
- **Aapki recommendation:** 3-large, ya kam se kam dono ka ek chhota test — 20 asal customer queries le kar dono models per retrieval hit-rate compare karo (yeh evaluation ka tareeqa Lesson 13 mein seekhenge).
...
|---------|---------------|
| Cosine score ko percentage samajhna ("0.45 = 45% match") | Score sirf **relative** hai — pairs ka muqabla karo, absolute matlab mat nikalo |
| Index aur query ke liye alag models use karna | Same model dono jagah — hamesha. Model badla to sab dobara embed karo |
| "Zyada dimensions = hamesha behtar" | English-only ke liye 3-small ka 1,536 kaafi hai — 2.3 points ke liye 6.5× qeemat mat do |
| Multilingual users ko English benchmark (MTEB) dikha kar model chunna | Multilingual ke liye **MIRACL** dekho — wahan farq 10+ points ka hai |
...
| `dimensions` parameter chhote projects per istemal karna | Yeh scale ka tool hai (lakh+ chunks). Chhote corpus per default hi behtar hai |

---

## 9. Best Practices & Industry Tips

1. **Test with your own phrases before choosing a model.** The 40-line demo script is the cheapest model-evaluation you will ever run — five phrases from your client's actual domain tell you more than any benchmark table.
2. **Write the model name into your collection's metadata or docs.** Six months later, "which model embedded this?" must have an answer — otherwise you cannot query safely.
3. **Budget re-embedding from day one.** Content changes, models improve. If re-embedding the whole corpus is painful, your pipeline is wrong (Lesson 05 fixes this).
4. **When a client's users are multilingual, say so in the proposal.** Model choice is a line-item decision with a cost attached — surfacing it early builds trust and protects retrieval quality later.
...
|---|---|---|
| `Error generating embedding: ... api_key` | `.env` missing ya `OPENAI_API_KEY` set nahi | Backend folder mein `.env` banao (`.env.example` copy karo) — key kabhi code mein nahi |
| Script chalta hai lekin bohat slow | Har phrase alag request ban rahi hai / network | Script `batch_generate` use karti hai — ek hi request; slow ho to network check karo |
| `ModuleNotFoundError: src` | Script backend folder ke bahar se chalayi | `cd backend` kar ke `.venv/bin/python scripts/similarity_demo.py` |
...
Edit the `PHRASES` list in `scripts/similarity_demo.py`, run it, and fill this in:
...
**Q3.** A bot serves users who write in both English and Roman Urdu. Which benchmark difference matters most for model choice?
- (a) MTEB (62.3% vs 64.6%)
- (b) MIRACL (44.0% vs 54.9%)
- (c) Price per token
...
**Q3: (b)** — MIRACL measures multilingual retrieval, and the gap there is large (44.0 → 54.9). MTEB is English-focused and the gap is small (2.3 points).

**Q4: (c)** — It's a documented small-model limitation, and our demo caught it on real data. The design responses: choose 3-large for multilingual use, or test with real queries before committing (Lesson 13 covers evaluation).
...
4. **Small models have a real multilingual gap** — our own demo showed shared language beating shared meaning (0.272 > 0.245); MIRACL quantifies it (44.0% vs 54.9%)
5. **Same model for index and query, always** — changing models means re-embedding everything
...
A: By language and budget. For English-only corpora, 3-small is 6.5× cheaper with only a 2.3-point MTEB gap. For multilingual use, 3-large's MIRACL advantage (54.9% vs 44.0%) is usually worth the cost. I'd validate with a small retrieval test on real queries before committing.
...
A: On our own system, a Roman Urdu phrase matched an unrelated same-language phrase more strongly than its exact English translation — shared language outweighed shared meaning. That's consistent with 3-small's MIRACL score, and it's why multilingual products should test retrieval with real user queries.

---

## 15. Sources

*All claims re-checked against the original sources on **4 August 2026**.*

| Claim used in this lesson | Source | Checked |
|---|---|---|
| 3-small: 1,536 dims default · 3-large: 3,072 · MTEB 62.3% / 64.6% · cosine recommended · vectors normalized (length 1) · dimensions-shortening supported | OpenAI embeddings guide (official): [platform.openai.com/docs/guides/embeddings](https://platform.openai.com/docs/guides/embeddings) | 4 Aug 2026 |
| Pricing $0.02 / $0.13 per 1M tokens · MIRACL 44.0% / 54.9% (ada-002: 31.4%) · 3-large-at-256-dims beats ada-002 claim | OpenAI announcement (official): [openai.com/index/new-embedding-models-and-api-updates](https://openai.com/index/new-embedding-models-and-api-updates/) | 4 Aug 2026 |
| Our stack: `text-embedding-3-small`, 1,536 dims, Qdrant `Distance: Cosine`, collection `book_content` | Verified directly in this book's source: `backend/src/services/embedding_service.py`, `backend/scripts/setup_qdrant.py` | 4 Aug 2026 |
| All similarity scores in §4–§5 | Measured — actual run of `backend/scripts/similarity_demo.py` on 4 Aug 2026. Scores may drift slightly between runs; the **ranking** is the claim, not the third decimal | 4 Aug 2026 |
| Travel-agency cost math (§7): 100K tokens × $0.13/1M = $0.013 | Arithmetic from the official price above | 4 Aug 2026 |
